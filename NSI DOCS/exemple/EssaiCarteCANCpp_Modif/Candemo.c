...
{	
	...
	// Valeurs des 8 octets de données
	canObj.data[0] = 0xF8;
	canObj.data[1] = 0xFF;
	canObj.data[2] = 0x3E;
	canObj.data[3] = 0x80;
	...
	cr = Ic_SetRxMask( hCanal, 0x000, 0x000, _CAN_EXT );

	Echo( "Ic_SetRxMask( Mask=0x000, Val=0x000 ) : %s.", GetCodeString(cr));
	if( cr != _OK ) done = TRUE;
	...
	{
		Echo( " " );
		Echo( " Press [C] key to transmit ID %03Xh ", CAN_IDENT );
	...
	while( !done )
	{
		...
		case 'C' :
			...
			//
			for( i=0; i<longdonnee; i++ ) data[i] = canObj.data[i];
